...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            #print(f"Next action distribution: {next_action_distribution}")
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        #print(f"Alpha Item: {alpha.item()}, Log Prob: {log_prob.mean().item()}, Target Entropy: {self.target_entropy}, Alpha Loss: {alpha_loss.item()}")
    ...
    def update(
        ...
        result["temperature"] = self.get_temperature().item() if torch.is_tensor(self.get_temperature()) else self.get_temperature()
